...
function ManualRecon() {
  ...
  const total = state.issues.coreTotal + state.issues.partyTotal;

  const selTransLen =
    state.issues.selectedCoreTrans.length +
    state.issues.selectedPartyTrans.length;

  const handleClick = () => {
    if (total === 0 && selTransLen > 0) {
      dispatch({
        type: "updateDialog",
        payload: {
          show: true,
          ...
          title: "Are you sure?",
          description:
            "Click confirm to manually reconcile selected transactions.",
          action: sendTrans,
        },
      });
    }
    ...
    if (total !== 0) {
      dispatch({
        ...
        payload: {
          ...
          type: "error",
          hideAction: true,
          title: "None zero balance?",
          description:
            "Selected transactions must amount to zero before they can be manaully reconciled.",
          action: sendTrans,
        },
      });
    }
